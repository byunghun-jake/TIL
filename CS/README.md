...
1. 사이트에 접속할 때, 브라우저는 서버가 누구인지 확인하는 과정을 거칩니다. 이를 handshake라고 하는데, 이 과정에서 브라우저는 서버에게 랜덤한 데이터를, 서버는 브라우저에게 랜덤한 데이터와 암호화된 인증서를 전달합니다.
...
2. 브라우저에 저장되어있는 CA의 공개키로 암호화된 인증서를 복호화합니다.
   ...
   - 복호화 시, 인증서와 함께 서버의 공개키를 얻을 수 있습니다.
     ...
     `암호화된 인증서 ==(복호화)==> 인증서 + 서버의 공개키`

   - 비대칭키를 이용해 암호화하여 통신하는 방식은, 대칭키를 이용해 통신하는 방식에 비해 시간이 많이 걸립니다. 따라서, 서버의 공개키는 데이터를 주고받을 때 사용할 대칭키를 만드는 데 활용됩니다.

3. 클라이언트는 handshake 과정에서 주고 받았던 클라이언트의 랜덤한 데이터 + 서버의 랜덤한 데이터 + 서버의 공개 키를 이용해서 임시키를 발급하고, 서버에게 전달합니다.
   - 일련의 과정을 거쳐 대칭키를 생성합니다. (마법)
...
1. 컴퓨터와 컴퓨터가 네트워크를 이용해 통신을 할 때, 3 단계를 거친다.

   악수 => 전송 => 세션 종료

2. 비대칭키 방식은 컴퓨팅 파워를 많이 사용하여, 성능상 단점이 많다.
...
3. 악수(Handshake) 과정
   ...
   1. [Client] 서버에 접속한다.
      ...
      - 클라이언트 측에서 전달하는 정보
        ...
        1. 클라이언트가 생성한 랜덤한 데이터

        2. 클라이언트 측에서 사용할 수 있는 암호화 방식

           통신을 위해 필요한 암호화 방식을 서로 알아야 하기 때문에

        3. 세션 아이디

           기존에 활성화 된 세션이 있다면, 해당 세션을 재활용하기 위해

   2. [Server] 클라이언트에게 응답한다.
      ...
      - 서버 측에서 전달하는 정보
        ...
        2. 통신할 암호화 방식
           ...
           클라이언트에서 전달한 암호화 방식들 중 서버도 사용할 수 있는 암호화 방식

        3. 인증서

   3. [Client] 인증서 확인

      1. 서버의 인증서가 CA에 의해 발급된 것인지 확인하기 위해, Client에 내장된 CA 리스트를 확인한다.

         - CA 리스트에 인증서가 없다면, 사용자에게 경고 메시지를 출력한다.

      2. 인증서가 CA에 의해 발급된 것인지 확인하기 위해, 클라이언트에 내장된 CA의 공개키를 이용해서 인증서를 복호화한다.

         - 복호화에 성공했다면, CA의 개인키로 암호화된 문서라는 것이 증명된 것

           > 이 인증서는 CA에서 발급된 게 맞구나!

         - 인증서 내부에 서버의 개인 키가 들어있다.

      3. 클라이언트의 랜덤 데이터 + 서버의 랜덤 데이터를 조합하여 "pre master secret"이라는 키를 생성한다.

         암호화된 통신을 할 때 사용할 키이기에 절대 노출되어서는 안된다.

      4. 서버의 공개키를 이용해 "pre master secret" 값을 암호화한 뒤, 서버로 전송한다.

   4. [Server] 클라이언트가 전송한 "pre master secret" 값을 서버의 개인키로 복호화한다.

   5. [Client, Server] 일련의 과정을 거쳐 "pre master secret"을 "master secret"으로 만든다.

      - master secret은 session key를 생성하는데, 이 session key 값을 이용하여 주고받을 데이터를 암복호화한다.

   6. [Client, Server] 핸드쉐이크 종료를 서로에게 알린다.
